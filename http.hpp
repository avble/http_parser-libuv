...
public:
  ...
  static void call_soon(F f, Args... args)
  {
    ...
    timer_handle->data =
      (void*)(new wrapper_(std::forward<F>(f), std::forward<Args>(args)...));
...
void
start_server(unsigned short port, std::function<void(response&)> _handler)
{
  ...
  public:
    void do_read_request()
    {
      ...
      static http_cb llhttp_on_message = [](http_parser* req) -> int {
        ...
        auto do_write_ =
          [func_ = std::bind(&http_session::do_write_response, self, res)]() {
            func_();
          };
      ...
      static uv_read_cb read_cb =
        [](uv_stream_t* tcp, ssize_t nread, const uv_buf_t* buf) {
          ...
          if (nread > 0) {
            ...
            size_t return_size = http_parser_execute(
              &(self->parser), &(self->settings), buf->base, nread);
    ...
    void do_write_response(response res)
    {
      auto do_write_ = [self = shared_from_this()](response res) {
        ...
        uv_write(&self->write_res,
                 (uv_stream_t*)&self->tcp_stream,
                 &resbuf,
                 1,
                 [](uv_write_t* writer, int status) {
  ...
  static auto on_connect = [&](uv_stream_t* handle, int status) {
    std::tuple<std::function<void(response&)>>* p =
      (std::tuple<std::function<void(response&)>>*)handle->data;

    auto http_connection =
      std::shared_ptr<http_session>{ new http_session(std::get<0>(*p)),
                                     [](http_session* p) {
                                       //    std::cout << "[DEBUG] http_session
                                       //    is deleted.\n";
                                       delete p;
                                     } };
  ...
  int rc = uv_listen(
    (uv_stream_t*)&server,
    MAX_WRITE_HANDLES,
    [](uv_stream_t* server, int status) { on_connect(server, status); });
